	...
	async function handleSubmit(e) {
		...
		try {
			...
			if (!result.user.emailVerified) {
				alert('Email kamu belum diverifikasi! Silakan cek email dan klik link verifikasi.');
				await signOut(auth); // logout supaya sessionnya terhapus
				return;
			}
			// Email sudah verifikasi, boleh lanjut login
			navigate('/');
		} catch (error) {
			...
			<form onSubmit={handleSubmit}>
				...
				<div className="mb-2">
					...
					<div className="flex justify-end mt-1">
						<a
							href="#"
							className="text-sm text-[#446ed7] hover:underline">
							Lupa password ?
						</a>
					</div>
				</div>
