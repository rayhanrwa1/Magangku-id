...
function CreateProfile() {
	...
	const uploadPhoto = async (uid) => {
		...
		try {
			...
			const fileRef = storageRef(storage, `mitra_photos/${uid}/${Date.now()}.jpg`);
	...
	const handleSubmit = async () => {
		...
		if (!agreeTerms) {
			showToastTopRight('warning', 'Anda harus menyetujui perjanjian kemitraan');
			return;
		}
		...
		try {
			...
			showToastTopRight('success', isEditMode ? 'Profil berhasil diperbarui!' : 'Profil berhasil dibuat!');
			...
			<div className="flex-1 flex flex-col">
				...
				<main className="p-4 md:p-6 space-y-6">
					<div className="flex items-center justify-between">
						<div>
							<h1 className="text-2xl font-bold text-slate-800">{isEditMode ? 'Edit Profil Mitra' : 'Buat Profil Mitra'}</h1>
							<p className="text-slate-600 text-sm mt-1">Lengkapi informasi profil mitra Anda untuk proses verifikasi</p>
						</div>
					</div>
					...
					<div className="space-y-6">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="md:col-span-2">
									...
									<input
										...
										className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 ${errors.name ? 'border-red-300' : 'border-slate-300'}`}
									/>
								...
								<div className="md:col-span-2">
									...
									<textarea
										...
										className={`w-full px-4 py-2.5 border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-slate-500 ${errors.description ? 'border-red-300' : 'border-slate-300'}`}
									/>
									{errors.description && <p className="text-red-600 text-sm mt-1">{errors.description}</p>}
								</div>

								<div>
									<label className="block text-sm font-medium text-slate-700 mb-2">Tipe Vendor *</label>
									<select
										name="vendor_type"
								...
								<div>
									...
									<input
										...
										className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 ${errors.phone_number ? 'border-red-300' : 'border-slate-300'}`}
									/>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div className="md:col-span-2">
									...
									<textarea
										...
										className={`w-full px-4 py-2.5 border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-slate-500 ${errors.address ? 'border-red-300' : 'border-slate-300'}`}
									/>
						...
						<div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
							<div className="flex items-center gap-3 mb-6">
								...
								<h2 className="text-lg font-semibold text-slate-800">Foto / Logo</h2>
							</div>

							<div className="space-y-4">
								<label className="block text-sm font-medium text-slate-700">Upload Foto / Logo</label>
								...
								<label className="flex flex-col items-center justify-center w-full px-6 py-6 bg-slate-100 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-200 transition">
									<Camera
										...
										color="#475569"
									/>
									<p className="text-sm mt-2 text-slate-600">Klik untuk memilih foto atau logo</p>
								...
								{uploadingPhoto && <p className="text-blue-600 text-sm mt-1">Mengunggah foto...</p>}
							</div>
						</div>
						...
						<div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
							<div className="flex items-center gap-3 mb-6">
								...
								<h2 className="text-lg font-semibold text-slate-800">Media Sosial</h2>
							</div>
						...
						<div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
							<div className="flex items-center gap-2 flex-wrap">
								<label className="flex items-center gap-2 cursor-pointer">
									...
									/>
									<span className="text-sm font-medium text-slate-700">Saya menyetujui Perjanjian Kemitraan</span>
								</label>
								<button
									type="button"
				...
				<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
					<div className="bg-white rounded-xl p-6 max-w-lg w-full space-y-4">
						...
						<div className="space-y-1">
							<label className="block text-sm font-medium text-slate-700">Zoom: {Math.round(zoom * 100)}%</label>
							<input
								type="range"
								min={0.5}
								max={3}
			...
			{showAgreement && (
				<div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
					<div className="bg-white w-full max-w-2xl rounded-xl shadow-lg p-6 space-y-4">
						<h2 className="text-xl font-bold text-slate-800">Perjanjian Kemitraan Magang</h2>

						<div className="max-h-72 overflow-y-auto pr-2 space-y-4 text-slate-700 text-sm leading-relaxed">
							<p>
								Perjanjian ini mengatur hubungan antara <strong>Mitra Perusahaan</strong> dan platform <strong>Magangku</strong> dalam menyediakan lowongan magang bagi mahasiswa. Dengan menjadi mitra, perusahaan menyatakan memahami dan
								bersedia mematuhi ketentuan berikut:
							</p>

							<p>
								<strong>1. Kebenaran Data</strong>
								<br />
								Mitra bertanggung jawab atas kebenaran seluruh informasi yang diunggah ke sistem Magangku.
							</p>

							<p>
								<strong>2. Profesionalitas & Pembinaan</strong>
								<br />
								Mitra wajib memberikan lingkungan magang yang aman, edukatif, dan sesuai standar profesional.
							</p>

							<p>
								<strong>3. Komunikasi</strong>
								<br />
								Mitra wajib merespons lamaran dan komunikasi dari peserta magang maupun sistem Magangku secara wajar.
							</p>

							<p>
								<strong>4. Status Verifikasi Perusahaan</strong>
								<br />
								Admin Magangku berhak menyetujui atau mencabut verifikasi jika ditemukan pelanggaran.
							</p>

							<p>
								<strong>5. Privasi & Keamanan</strong>
								<br />
								Mitra wajib menjaga kerahasiaan data peserta magang dan tidak menyalahgunakannya untuk tujuan non-profesional.
							</p>

							<p>
								<strong>6. Penghentian Kemitraan</strong>
								<br />
								Magangku berhak menonaktifkan akun mitra jika melanggar ketentuan.
							</p>
						</div>
						...
						<div className="flex items-center gap-3 pt-2">
							<input
								...
								className="w-5 h-5 cursor-pointer"
							/>
							<label className="text-sm text-slate-700">Saya telah membaca dan menyetujui perjanjian kemitraan.</label>
						</div>
						...
						<div className="flex justify-end gap-3 pt-4">
							...
							<button
								onClick={() => setShowAgreement(false)}
								disabled={!agreeTerms}
								className={`px-4 py-2 rounded-lg transition-colors ${agreeTerms ? 'bg-slate-700 text-white hover:bg-slate-800' : 'bg-slate-300 text-slate-500 cursor-not-allowed'}`}>
								Setuju
							</button>
						</div>
					</div>
