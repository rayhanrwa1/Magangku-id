  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 flex items-center justify-between bg-white transition-colors ${
            ...
            <>
              {selectedCountry.img ? (
                ...
                <div className="w-6 h-6 bg-slate-200 rounded flex items-center justify-center">
                  <Global size="14" className="text-slate-400" />
                </div>
              )}
              <span className="text-slate-700">{selectedCountry.name}</span>
            </>
          ) : (
        ...
        <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-lg shadow-xl overflow-hidden">
          ...
          <div className="max-h-64 overflow-y-auto">
            {loading ? (
              ...
              filteredCountries.map((country) => (
                <button
                  ...
                  className={`w-full px-4 py-3 flex items-center gap-3 hover:bg-slate-50 transition-colors text-left ${
                    value === country.name ? "bg-slate-100" : ""
                  }`}
                >
                  {country.img ? (
                    <img
                    ...
                    <div className="w-8 h-8 bg-slate-200 rounded shadow-sm flex items-center justify-center">
                      <Global size="16" className="text-slate-400" />
                    </div>
                  )}
              ...
              <div className="p-4 text-center">
                <Global size="32" className="mx-auto text-slate-300 mb-2" />
                <p className="text-sm text-slate-500">
                  {searchTerm ? "Negara tidak ditemukan" : "Tidak ada data"}
                </p>
              </div>
            )}
          </div>
