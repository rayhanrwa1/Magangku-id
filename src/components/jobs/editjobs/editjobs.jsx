...
import JobProvinceSelect from '../createjobs/jobprovinceselect';
import { Building, Location, Global, Briefcase, Clock, CloseCircle } from 'iconsax-react';
...
export default function EditJob() {
	...
	const validateForm = () => {
		...
		if (!formData.employment_duration.trim()) err.employment_duration = 'Durasi wajib diisi';
		if (!formData.education_requirement.trim()) err.education_requirement = 'Kualifikasi pendidikan wajib diisi';
	...
	const handleSubmit = async () => {
		...
		try {
			...
			showToastTopRight('success', 'Lowongan berhasil diperbarui!');
			...
			showToastTopRight('error', 'Gagal memperbarui lowongan. Coba lagi nanti.');
		} finally {
			setSaving(false);
		}
	};
				...
				<div className="flex-1 flex flex-col min-w-0">
					...
					<main className="p-4 md:p-6 lg:p-8">
						<div className="bg-white border border-red-200 rounded-lg p-6 md:p-8 text-center max-w-2xl mx-auto">
							...
							<p className="text-gray-600 mb-6">Anda belum dapat mengedit lowongan pekerjaan sebelum akun mitra diverifikasi oleh admin. Mohon tunggu proses verifikasi.</p>
			...
			<div className="flex-1 flex flex-col min-w-0">
				...
				<main className="p-4 md:p-6 lg:p-8 space-y-6">
					...
					<div>
						...
						<p className="text-sm text-gray-600 mt-1">Perbarui informasi lowongan pekerjaan</p>
					</div>

					{/* Form Card */}
					<div className="bg-white rounded-lg border border-gray-200">
						<div className="bg-slate-800 px-6 py-4 border-b border-gray-200">
							<h2 className="text-lg font-semibold text-white">Informasi Lowongan</h2>
							<p className="text-slate-300 text-sm mt-0.5">Perbarui detail posisi yang tersedia</p>
						</div>
						...
						<div className="p-6 space-y-5">
								...
								<div>
									...
									<div className="relative">
										...
										<input
											...
											className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-slate-800 focus:border-slate-800 outline-none text-sm ${errors.title ? 'border-red-300' : 'border-gray-300'}`}
								...
								<div>
									...
									<div className="relative">
										...
										<input
											...
											onChange={handleChange}
											className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-slate-800 focus:border-slate-800 outline-none text-sm ${errors.category ? 'border-red-300' : 'border-gray-300'}`}
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									...
									<input
										...
										onChange={handleChange}
										className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-slate-800 focus:border-slate-800 outline-none text-sm ${errors.employment_duration ? 'border-red-300' : 'border-gray-300'}`}
										placeholder="3 bulan"
									/>
									{errors.employment_duration && <p className="text-red-600 text-xs mt-1">{errors.employment_duration}</p>}
								</div>

								<div>
									<label className="block text-sm font-medium text-gray-700 mb-1.5">
										Kualifikasi Pendidikan <span className="text-red-600">*</span>
									</label>
									<input
										name="education_requirement"
										value={formData.education_requirement}
										onChange={handleChange}
										className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-slate-800 focus:border-slate-800 outline-none text-sm ${errors.education_requirement ? 'border-red-300' : 'border-gray-300'}`}
										placeholder="S1 Teknik Informatika"
									/>
									{errors.education_requirement && <p className="text-red-600 text-xs mt-1">{errors.education_requirement}</p>}
								</div>
							</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									<label className="block text-sm font-medium text-gray-700 mb-1.5">Minimal IPK</label>
									<input
										name="gpa_min"
										type="number"
										step="0.01"
								...
								<div>
									<label className="block text-sm font-medium text-gray-700 mb-1.5">Dokumen yang Dibutuhkan</label>
									<input
							...
							<div>
								...
								<textarea
									...
									placeholder="Jelaskan tanggung jawab dan detail pekerjaan..."></textarea>
							</div>

							{/* Qualifications */}
							<div>
								<label className="block text-sm font-medium text-gray-700 mb-1.5">Kualifikasi & Persyaratan</label>
								<textarea
									name="qualifications"
									...
									rows="4"
									className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-800 focus:border-slate-800 outline-none text-sm resize-none"
									placeholder="Jelaskan kualifikasi yang dibutuhkan..."></textarea>
							</div>

							{/* Important Dates & Deadline */}
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									<label className="block text-sm font-medium text-gray-700 mb-1.5">Tanggal Penting</label>
									<textarea
								...
								<div>
									...
									<div className="relative">
										...
										<input
											...
											onChange={handleChange}
											className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-slate-800 focus:border-slate-800 outline-none text-sm ${errors.deadline ? 'border-red-300' : 'border-gray-300'}`}
										/>
									</div>
							...
							<div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
								<label className="flex items-start gap-3 cursor-pointer">
									...
									/>
									<div>
										<span className="text-sm font-medium text-gray-700 block">Lowongan Umum</span>
										<span className="text-xs text-gray-500">Centang jika lowongan dapat dilamar oleh semua kandidat (tidak terbatas pada mahasiswa tertentu)</span>
									</div>
								</label>
							</div>
